...
{
	int inetd = (*(struct slapd_args *)ptr).sa_inetd;
	ber_socket_t tcps  = (*(struct slapd_args *)ptr).sa_tcps;
...
int slapd_daemon( int inetd, ber_socket_t tcps )
{
	int rc;
	struct slapd_args *args = ch_malloc( sizeof( struct slapd_args ) );
	args->sa_inetd = inetd;
	args->sa_tcps = tcps;
